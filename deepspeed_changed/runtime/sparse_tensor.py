...
class SparseTensor(object):
    ...
    def __init__(self, dense_tensor=None, dtype=None):
    ...
    def to_dense(self):
        ...
        dense_tensor = torch.zeros(self.dense_size, dtype=self.values.dtype, device=self.values.device).cuda()
        dense_tensor = dense_tensor.view(-1).scatter_add(0, self.indices.cuda(), self.values.cuda())
        return dense_tensor.view(self.dense_size)
