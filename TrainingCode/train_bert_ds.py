...
def train(
    ...
    print("Using optimizer: ", model.optimizer)
    print("Using lr_scheduler: ", model.lr_scheduler)
    ...
    for step, batch in enumerate(data_iterator, start=start_step):
        ...
        if step % checkpoint_every == 0:
            custom_save_checkpoint(save_dir=exp_dir,
...
def divide_group_parameters(model: nn.Module):
    layers = OrderedDict()
    for name, param in model.named_parameters():
        layer_name = name.split('.')[1]  # 例如：layer1.weight → layer1
        if layer_name not in layers:
            layers[layer_name] = []
        layers[layer_name].append(param)
    return layers


def custom_save_checkpoint(self, save_dir, tag, client_state={}, exclude_frozen_parameters=False):
