...
import sys
sys.path.append('rckpt')
from rckpt.custom_deepspeed_init import custom_initialize
...
parser.add_argument('--dataset', metavar='DIR', nargs='?', default='imagenet',
                    help='path to dataset (default: imagenet)')
parser.add_argument('--save_checkpoint_dir', metavar='DIR', default='/data/lff/checkpoint',
                     help='path to saving checkpoint directory')
...
parser.add_argument('--wd', '--weight-decay', default=1e-4, type=float,
                    ...
                    dest='weight_decay')
parser.add_argument('--checkfreq', default=10, type=int,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model, optimizer, _, _ = custom_initialize(
        model = model,
        optimizer = optimizer,
        args = args,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        this_loss = train(train_loader, model, criterion, optimizer, scheduler, epoch, device, args)
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if i % args.checkfreq == 0:
            ...
            save_checkpoint({
                ...
                'optimizer' : optimizer.state_dict(),
                'scheduler' : scheduler.state_dict()
            }, args, epoch=epoch+1, iteration=i)
...
def save_checkpoint(state, args, is_best=False, epoch=0, iteration=0):
    filename = args.save_checkpoint_dir + '/{}_{}_{}.pth.tar'.format(args.arch, epoch, iteration)
