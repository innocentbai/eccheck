...
def asyn_ec(ec_queue, ec_param, ckpt_home, ec_home, num_param_group):
    ...
    while True:
        ...
        with open(pathckpt , "rb") as f:
            ...
            # 分块参数
            chunk_size = 512 * 1024 * 1024  # 128MB
            ...
            # 多进程编码
            pool = torch.multiprocessing.Pool(processes=num_workers, initializer=init_worker, initargs=ec_param)
            tasks = [(i, file, chunk_size, pathckpt, pathec) for i in range(total_chunks)]
            try:
                for i in range(0, len(tasks), num_workers):
                    batch = tasks[i:i + num_workers]
                    pool.starmap(encode_mmap_chunk, batch)
                pool.close()
                pool.join()
            except KeyboardInterrupt:
                logger.info(f"<Rank{getrank()}/{getworldsize()}> <iteration{iteration}> {file} KeyboardInterrupt KeyboardInterrupt KeyboardInterrupt KeyboardInterrupt!")
                ...
                logger.info(f"<Rank{getrank()}/{getworldsize()}> <iteration{iteration}> {file} Exception Exception Exception Exception!")
                while has_d_state_processes():
                    time.sleep(8)
                    logger.info(f"<Rank{getrank()}/{getworldsize()}> <iteration{iteration}> TIME SLEEP 8")
                pool.terminate()
